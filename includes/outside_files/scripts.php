...
function js_inc_function()
{
    if (!is_admin()) {
       ...
       wp_register_script('pmc_news', get_template_directory_uri() . '/js/jquery.li-scroller.1.0.js', array(
		...
		wp_enqueue_script('pmc_bxSlider');					        wp_register_script('pmc_script_php', get_template_directory_uri() . '/js/script.php', array(            'jquery'        ) ,true,true);  		wp_enqueue_script('pmc_script_php');			
...
function enqueue_css() {
    ...
    if( !is_admin()){
		...
		//wp_enqueue_style( 'prettyp');				if(isset($data['heading_font'])){
			wp_register_style('googleFont', 'http://fonts.googleapis.com/css?family='.$data['heading_font']['face'] ,'',NULL);
			wp_enqueue_style( 'googleFont');					}		if(isset($data['body_font'])){		
