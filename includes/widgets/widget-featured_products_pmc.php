...
class WooCommerce_widget_featured_products_Custom extends WP_Widget {
	...
	function WooCommerce_widget_featured_products_Custom() {
		...
		$this->woo_widget_name = __('Emporium Featured Products', 'woocommerce' );
...
?>
		...
		<li>
    	<a href="<?php echo esc_url( get_permalink( $r->post->ID ) ); ?>" title="<?php echo esc_attr($r->post->post_title ? $r->post->post_title : $r->post->ID); ?>"><img src="
				<?php 
					$thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($r->post->ID), 'footer_thumb');
					echo $thumbnail[0];
					//echo get_image_pmc(100,75,$r->post->ID);
				?>" />
    	</a>
