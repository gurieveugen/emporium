...
add_action( 'widgets_init', 'WooCommerce_widget_recent_reviews_Custom_widget' );

function WooCommerce_widget_recent_reviews_Custom_widget() {
	register_widget( 'WooCommerce_widget_recent_reviews_Custom' );
}

class WooCommerce_widget_recent_reviews_Custom extends WP_Widget {
	 ...
	 * @return void
	 */
	function WooCommerce_widget_recent_reviews_Custom() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass = 'widget_recent_reviews_custom';
		...
		$this->woo_widget_name = __('Emporium Recent Reviews', 'woocommerce' );
	 ...
	 */
	 function widget( $args, $instance ) {
		...
		$cache = wp_cache_get('widget_recent_reviews_custom', 'widget');
		...
		if ( $comments ) :
			...
			foreach ( (array) $comments as $comment) :
				...
				echo get_image_pmc(100,75,$comment->comment_post_ID).'</a>';
		...
		wp_cache_set('widget_recent_reviews_custom', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_recent_reviews_custom']) ) delete_option('widget_recent_reviews_custom');
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_recent_reviews_custom', 'widget');
	}
