			...
			$count = 0;
				...
				<?php if(isset($postmeta["video_active"][0]) && $postmeta["video_active"][0] == 1) { ?>
					...
					<div class="recentimage">
						...
						<div class="image">
							<div class="loading"></div>
							<?php if (has_post_thumbnail( $post->ID )) echo get_image_pmc(300,225,$post->ID); else echo '<img src="'.woocommerce_placeholder_img_src().'" alt="Placeholder" width="'.$woocommerce->get_image_size('shop_catalog_image_width').'px" height="'.$woocommerce->get_image_size('shop_catalog_image_height').'px" />'; ?>
						</div>
					</div>						
					<?php  }  ?>
				</a>
