...
add_action( 'widgets_init', 'WooCommerce_widget_recent_products_Custom_widget' );



function WooCommerce_widget_recent_products_Custom_widget() {
	register_widget( 'WooCommerce_widget_recent_products_Custom' );
}

class WooCommerce_widget_recent_products_Custom extends WP_Widget {
	 ...
	 * @return void
	 */
	function WooCommerce_widget_recent_products_Custom() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass = 'widget_recent_products_custom';
		...
		$this->woo_widget_name = __('Emporium Recent Products', 'woocommerce' );
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('widget_recent_products_custom', 'widget');
...
?>
		...
		<a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>">
			<?php echo get_image_pmc(100,75,$r->post->ID);?> 		
		</a>
		<a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>">	
...
<?php
		...
		wp_cache_set('widget_recent_products_custom', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_recent_products_custom']) ) delete_option('widget_recent_products_custom');

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete('widget_recent_products_custom', 'widget');
	}
